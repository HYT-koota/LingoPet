...
const MAX_TRANSLATION_BACKFILL = 12;
...
const Notebook: React.FC<NotebookProps> = ({ onBack }) => {
  ...
  useEffect(() => {
    ...
    const backfillMissingTranslations = async (allWords: WordEntry[]) => {
      const missingTranslationWords = allWords
        .filter(w => !w.translation?.trim())
        .slice(0, MAX_TRANSLATION_BACKFILL);

      for (const word of missingTranslationWords) {
        if (isCancelled) return;
        const translated = (await generateShortTranslation(word.word, word.definition)).trim();
          ...
          if (!isCancelled) {
            setWords(prevWords =>
              prevWords.map(prevWord =>
                prevWord.id === word.id ? { ...prevWord, translation: translated } : prevWord
              )
            );
          }
        } catch (error) {
          console.error(`[Notebook] Failed to update translation for "${word.word}":`, error);
        }
      }
    };
    ...
    const loadWords = async () => {
      ...
      const sortedWords = allWords.sort((a, b) => b.addedAt - a.addedAt);
      setWords(sortedWords);
      void backfillMissingTranslations(sortedWords);
    };
  ...
  return (
    <div className="h-full flex flex-col bg-gray-50 animate-pop">
      ...
      {/* List Area */}
      <div className="flex-1 overflow-y-auto app-main-scroll px-6 py-6 bg-dot-pattern">
